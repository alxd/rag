...
class UpSetGUI:
    ...
    def setup_ui(self):
        ...
        main_frame.rowconfigure(4, weight=1)  # Results frame row
        
        # --- Merge CSVs section ---
        merge_frame = ttk.LabelFrame(main_frame, text="Merge CSV Files", padding="5")
        merge_frame.grid(row=0, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0,5))
        self.merge_label = ttk.Label(merge_frame, text="No files selected", foreground="gray")
        self.merge_label.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 10), pady=5)
        ttk.Button(merge_frame, text="Select & Merge CSVs", command=self.merge_csv_files).grid(row=0, column=1, padx=(0, 10), pady=5)
        ...
        self.color_criteria_cb.grid(row=0, column=0, sticky=tk.W)
        ...
        self.group_by_same_color_cb.grid(row=0, column=5, padx=(10,0))
        self.group_by_same_color_cb.state(['disabled'])
        # Add label wrap width entry
        ttk.Label(color_frame, text="Label wrap width:").grid(row=0, column=6, padx=(10,0))
        ...
        self.wrap_width_entry.grid(row=0, column=7, padx=(2,0))
        
        # Process button and progress
        button_frame = ttk.Frame(main_frame)
        button_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
    ...
    def merge_csv_files(self):
        ...
        if not file_paths:
            return
        try:
            # Use encoding detected as 'windows-1252' for these files
            dfs = [pd.read_csv(fp, encoding='windows-1252') for fp in file_paths]
            ...
            self.merge_label.config(text=f"Merged: {', '.join([os.path.basename(fp) for fp in file_paths])}", foreground="black")
            self.process_btn.config(state='normal')
        except Exception as e:
