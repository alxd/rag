...
class UpSetGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Checkbutton(color_frame, text="Color criteria by word families", variable=self.use_colors).grid(row=0, column=0, sticky=tk.W)
        ttk.Label(color_frame, text="Min shared letters:").grid(row=0, column=1, padx=(10,0))
        self.min_letters_var = tk.StringVar(value="3")
    ...
    def create_color_mapping(self, all_concepts):
        """Assign a unique color to each concept string, globally and stably."""
        self.color_mapping = {}
        all_concepts_sorted = sorted(all_concepts)
        colors = [
            '#FF0000', '#0000FF', '#008000', '#FF8C00', '#8B008B', '#8B4513',
            '#2F4F4F', '#800080', '#DC143C', '#4B0082', '#B22222', '#191970',
            # Add more if needed
        ]
        for idx, concept in enumerate(all_concepts_sorted):
            self.color_mapping[concept] = colors[idx % len(colors)]
    ...
    def process_data(self):
        try:
            ...
            # Build global color mapping from all concepts in the dataset
                all_concepts = set()
