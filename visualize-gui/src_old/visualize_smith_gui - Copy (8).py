...
class UpSetGUI:
    ...
    def generate_stats_files(self, blocks, df, color_mapping, outdir, param_labels):
        try:
            ...
            # --- Color Groups section at the top ---
            doc.add_heading("Color Groups", level=1)
            # Insert 4 images in a table, side by side
            ...
            # Color Groups table: 4 columns (remove Color, expand Parameters)
            color_table = doc.add_table(rows=1, cols=4)
            color_table.autofit = False
            color_table.allow_autofit = False
            ...
            # Set column widths: Count very narrow, Parameters double
            color_table.columns[0].width = Inches(1.2)
            color_table.columns[1].width = Inches(0.4)
            color_table.columns[2].width = Inches(2.2)
            color_table.columns[3].width = Inches(3.2)
            for color, concepts in color_groups.items():
