...
class UpSetGUI:
    ...
    def setup_ui(self):
        ...
        threshold_frame.grid(row=3, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=(5,2))
    ...
    def aggregate_results(self):
        ...
        import difflib
        import traceback
        self.aggregate_status_label.config(text="Scanning folders...", foreground="blue")
        self.root.update_idletasks()
        parent = self.aggregate_folder
        if not parent or not os.path.isdir(parent):
            self.aggregate_status_label.config(text="Invalid parent folder.", foreground="red")
            return
        ...
        if not valid_folders:
            self.aggregate_status_label.config(text="No valid result folders found.", foreground="red")
            return
        self.aggregate_status_label.config(text=f"Found {len(valid_folders)} valid folders. Extracting data...", foreground="blue")
        self.root.update_idletasks()
        ...
        if not all_concepts:
            self.aggregate_status_label.config(text="No concepts found in any folder.", foreground="red")
            return
        ...
        # Step 4: Assign unique colors to canonical concepts
        color_palette = [
            '#800000', '#FF8C00', '#228B22', '#8B008B', '#A0522D', '#2E8B57', '#9932CC', '#FFD700',
            '#556B2F', '#C71585', '#8B4513', '#20B2AA', '#B22222', '#FF4500', '#6A5ACD', '#D2691E',
            '#006400', '#708090', '#FF6347', '#483D8B', '#000000', '#808000', '#8B0000', '#FF1493',
        ]
        concept_color = {c: color_palette[i % len(color_palette)] for i, c in enumerate(canonical_concepts)}
        # Step 5: Compute overlap/uniqueness using canonical groups
        ...
        try:
            ...
            doc.add_page_break()
            # Table of all canonical concepts (color-coded, with variants)
            doc.add_heading("All Concepts (Grouped, Color-coded)", level=2)
            concept_table = doc.add_table(rows=1, cols=3)
            concept_table.rows[0].cells[0].text = "Canonical Concept"
            concept_table.rows[0].cells[1].text = "Color"
            concept_table.rows[0].cells[2].text = "Variants"
            for c in canonical_concepts:
                row = concept_table.add_row().cells
                run = row[0].paragraphs[0].add_run(c)
                color = concept_color[c]
                if color.startswith('#') and len(color) == 7:
                    r, g, b = tuple(int(color[j:j+2], 16) for j in (1, 3, 5))
                    run.font.color.rgb = RGBColor(r, g, b)
                row[1].text = color
                row[2].text = ", ".join(group_variants[c])
            doc.add_page_break()
            # Overlap table
            ...
            for row in overlap_table:
                ...
                doc_row[0].text = row[0]
                for i, present in enumerate(row[1:]):
                    doc_row[i+1].text = "✔" if present else ""
            # Percentage overlap summary
            doc.add_heading("Overlap Summary", level=2)
            ...
            doc.save(out_path)
            self.aggregate_status_label.config(text=f"Aggregation complete. Saved to {out_path}", foreground="green")
        except Exception as e:
            tb = traceback.format_exc()
            self.aggregate_status_label.config(text=f"Error generating DOCX: {e}", foreground="red")
            print(tb)
